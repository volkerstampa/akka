...
trait TcpHelper { this: TestKitBase ⇒
  ...
  def connect(serverAddress: InetSocketAddress): (Subscriber[ByteString], Publisher[ByteString]) = {
    ...
    val ts = SubscriberSource[ByteString]
    val tp = PublisherSink[ByteString]
    val outgoing = FlowFrom[ByteString].withSource(ts)
    val incoming = FlowFrom[ByteString].withSink(tp)
    val us = UndefinedSource[ByteString]
    val up = UndefinedSink[ByteString]
    val pipeline = PartialFlowGraph { implicit b ⇒
      import FlowGraphImplicits._
      // FIXME #15946 remove broadcast
      outgoing ~> Broadcast[ByteString](minimumOutputCount = 1) ~> up
      us ~> Broadcast[ByteString](minimumOutputCount = 1) ~> incoming
    }
    tcpProbe.send(IO(StreamTcp), StreamTcp.Connect(pipeline, serverAddress))

    val outgoingConnection = tcpProbe.expectMsgType[StreamTcp.OutgoingTcpConnection]

    (outgoingConnection.pipeline.getSourceFor(ts), outgoingConnection.pipeline.getSinkFor(tp))
  }

  def bind(connectionHandler: FlowWithSink[StreamTcp.IncomingTcpConnection, StreamTcp.IncomingTcpConnection],
  ...
  def echoServer(serverAddress: InetSocketAddress = temporaryServerAddress): EchoServer = {
    val foreachSink = ForeachSink[IncomingTcpConnection] { conn ⇒
      val ts = SubscriberSource[ByteString]
      val tp = PublisherSink[ByteString]
      val ms = conn.tcpFlow.withSource(ts).withSink(tp).run()
