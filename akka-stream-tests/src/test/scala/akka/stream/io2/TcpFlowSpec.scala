...
object TcpFlowSpec {
  ...
  def foldToFuture[I, O](flow: FlowWithSource[I, I], initial: O, f: (O, I) ⇒ O)(implicit materializer: FlowMaterializer): Future[O] = {
    val futureSink = FutureSink[O]
    val mf = flow.transform[O]("fold", () ⇒ {
      new Transformer[I, O] {
        var res = initial

        override def onNext(element: I) = {
          res = f(res, element)
          Nil
        }
    ...
    futureSink.future(mf)
  }

  def foldByteStringsToFuture(flow: FlowWithSource[ByteString, ByteString])(implicit materializer: FlowMaterializer): Future[ByteString] = {
    foldToFuture[ByteString, ByteString](flow, ByteString.empty, { case (res, elem) ⇒ res ++ elem })
  }
}
...
class TcpFlowSpec extends AkkaSpec with TcpHelper {
  ...
  "Outgoing TCP stream" must {
    ...
    "be able to write a sequence of ByteStrings" in {
      ...
      FlowFrom(tcpPublisher).consume()

      FlowFrom(testInput).publishTo(tcpSubscriber)
    ...
    "be able to read a sequence of ByteStrings" in {
      ...
      val resultFuture = foldByteStringsToFuture(FlowFrom(tcpPublisher))
  ...
  "TCP listen stream" must {
    ...
    "be able to implement echo" in {
      ...
      FlowFrom(testInput).toPublisher().subscribe(tcpSubscriber)
      val resultFuture = foldByteStringsToFuture(FlowFrom(tcpPublisher))
    ...
    "work with a chain of echoes" in {
      ...
      FlowFrom(testInput).toPublisher().subscribe(tcpSubscriber1)
      tcpPublisher1.subscribe(tcpSubscriber2)
      tcpPublisher2.subscribe(tcpSubscriber3)
      val resultFuture = foldByteStringsToFuture(FlowFrom(tcpPublisher3))
