...
private[akka] object TcpStreamActor {
  ...
  def outboundProps(connectCmd: Connect, requester: ActorRef, pipeline: PartialFlowGraph, materializer: FlowMaterializer): Props =
    Props(new OutboundTcpStreamActor(connectCmd, requester, pipeline, materializer)).withDispatcher(materializer.settings.dispatcher)
 ...
 * INTERNAL API
 */
private[akka] class OutboundTcpStreamActor(val connectCmd: Connect, val requester: ActorRef, pipeline: PartialFlowGraph, _materializer: FlowMaterializer)
  extends TcpStreamActor()(_materializer) {
  import TcpStreamActor._
  ...
  override def receive = {
    case c @ Connected(remoteAddress, localAddress) ⇒
      ...
      tcpOutputs.setConnection(connection)
      tcpInputs.setConnection(connection)
      val source = pipeline.undefinedSources.head.asInstanceOf[UndefinedSource[ByteString]]
      val sink = pipeline.undefinedSinks.head.asInstanceOf[UndefinedSink[ByteString]]
      val mg = FlowGraph(pipeline) { implicit b ⇒
        b.attachSource(source, PublisherSource(processor))
        b.attachSink(sink, SubscriberSink(processor))
      }.run()
      requester ! StreamTcp.OutgoingTcpConnection(remoteAddress, localAddress, mg)
